...
subtest "Basic setup with title", {
    plan 2;
...
subtest "Basic setup with border = False", {
    plan 2;
    my $table = $original-table.clone;
    $table.border(False);
    basic-setup($table);
...
subtest "Basic setup with header = False", {
    plan 2;
    my $table = $original-table.clone;
    $table.header(False);
    basic-setup($table);
...
subtest "Basic setup with hrules = NONE", {
    plan 2;
    my $table = $original-table;
    $table.clone.hrules(NONE);
    basic-setup($table);
}
